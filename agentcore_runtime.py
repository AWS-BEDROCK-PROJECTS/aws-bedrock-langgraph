...
# Import AgentCore runtime and create the application instance.
# This enables the script to be executed inside Bedrock AgentCore.
...
def load_faq_csv(path: str) -> List[Document]:
    """Load FAQ entries from a CSV file and convert them into Document objects."""
    docs = []
...
# Load the FAQ dataset once at startup.
docs = load_faq_csv("./lauki_qna.csv")

# Initialize a sentence-transformer embedding model for semantic search.
emb = HuggingFaceEmbeddings(
    model_name="sentence-transformers/all-MiniLM-L6-v2",
)

# Split the FAQ documents into chunks to improve vector search performance.
...
def agent_invocation(payload, context):
    ...
    # Extract query from payload. AgentCore payloads typically carry the user prompt
    # under the `prompt` key.
    query = payload.get("prompt", "No prompt found in input")
    
    # Invoke the LangChain agent with the extracted user query.
    result = agent.invoke({"messages": [("human", query)]})
    
    print("Result:", result)
    
    # Return only the final answer string in the response payload.
